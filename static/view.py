...
DATA_DIR = '/home/kaloix/home-sensor/data/'
RECORD_DAYS = 7
SUMMARY_DAYS = 365
...
series_list = list()
sw = codecs.getwriter('utf-8')(sys.stdout.detach())
...
def timer(tag):
	...
	yield
	logging.debug('{} done in {:.3f}s'.format(tag, time.perf_counter()-start))
...
def main():
	...
	group = cgifs['group'].value
	with timer('import sensors'):
		...
		for device in devices:
			for kind, attr in device['output'].items():
				if attr['group'] != group:
					continue
				if kind == 'temperature':
					series_list.append(Temperature(
						attr['low'],
						attr['high'],
						attr['name'],
					...
					series_list.append(Switch(
						attr['name'],
						device['input']['interval'],
						attr['fail-notify']))
	values = detail_html(series_list)
	with timer('plot history'):
		svgdata = plot_history(series_list)
	b64 = codecs.encode(svgdata, 'base64').replace(b'\n', b'').decode()
	sw.write('Content-Type: text/html;charset=utf-8\n\n')
	sw.write(values)
	sw.write('<img src=\'data:image/svg+xml;base64,')
	sw.write(b64)
	sw.write('\'>')
...
def plot_history(series_list):
	...
	# return file
	svgstream = io.BytesIO()
	with timer('savefig'):
		matplotlib.pyplot.savefig(svgstream, format='svg', bbox_inches='tight')
	svgbytes = svgstream.getvalue()
